  ...
  describe '#to_hash' do
    context 'when the Class is created with id' do
      it 'should return a hash including the proper keys' do
        storage_time_hash = storage_time.to_hash
        expect(storage_time_hash).to include('id', 'duration_time', 'start_time',
                                             'event', 'storage_date')
      end

      it 'should have the key id' do
        obj_hash = storage_time.to_hash
    ...
    context 'when the Class is not created with id' do
      it 'should not have the key id' do
        new_obj = storage_time
        new_obj.id = nil
        storage_time_hash = new_obj.to_hash
        expect(storage_time_hash).not_to have_key('id')
      end
    end
